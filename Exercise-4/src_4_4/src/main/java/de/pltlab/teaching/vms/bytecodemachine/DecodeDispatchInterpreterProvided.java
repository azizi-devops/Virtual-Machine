...
public class DecodeDispatchInterpreterProvided extends Interpreter {
	...
	private BasicBlock previousBlock = null;
	private final Map<String, Integer> edgeProfiles = new LinkedHashMap<>();
	...
	private void recordBasicBlock(int endPc) {
		...
		BasicBlock currentBlock = basicBlocks.get(key);

		if (currentBlock == null) {
			currentBlock = new BasicBlock(basicBlocks.size(), currentBlockStartPc, endPc);
			basicBlocks.put(key, currentBlock);
		}

		currentBlock.profile++;

		if (previousBlock != null) {
			String edgeKey = "BB" + previousBlock.id + " -> BB" + currentBlock.id;
			edgeProfiles.put(edgeKey, edgeProfiles.getOrDefault(edgeKey, 0) + 1);
		}
...
// for ManyBlocks
	...
	public String report() {
		...
		builder.append("\n");
		builder.append("Edge Profile\n");
		builder.append("============\n");

		for (Map.Entry<String, Integer> edge : edgeProfiles.entrySet()) {
			builder.append(edge.getKey())
					.append(": ")
