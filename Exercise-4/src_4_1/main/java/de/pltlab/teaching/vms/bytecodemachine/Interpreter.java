...
public abstract class Interpreter {
	...
	public Interpreter(File file) throws IOException {
		...
		parameters = BmClassVisitor.getMainMethodParameters(file);
	}
	...
	/**
	 * Initialize the arguments for the method interpretation to the passed values.
	 * A String array is used because that allows passing through the command line
	 * arguments of the BytecodeMachine's main method.
	 * 
	 * @param args
	 */
	public void setArguments(String[] args) {
		...
		for (int i = 0; i < parameters.length; i++) {
			...
			switch(type.getSort()) {
			case Type.BOOLEAN:
				setLocalVariable(varIndex, Boolean.parseBoolean(args[i]) ? 1 : 0);
				varIndex += 1;
				break;
			case Type.BYTE:
				setLocalVariable(varIndex, Byte.parseByte(args[i]));
				varIndex += 1;
				break;
			case Type.CHAR:
				setLocalVariable(varIndex, args[i].charAt(0));
				varIndex += 1;
				break;
			case Type.SHORT:
				setLocalVariable(varIndex, Short.parseShort(args[i]));
				varIndex += 1;
				break;
			case Type.INT:
				setLocalVariable(varIndex, Integer.parseInt(args[i]));
				varIndex += 1;
				break;
			case Type.LONG:
				setLocalVariable(varIndex, Long.parseLong(args[i]));
				varIndex += 2;
				break;
			case Type.FLOAT:
				setLocalVariable(varIndex, Float.parseFloat(args[i]));
				varIndex += 1;
				break;
			case Type.DOUBLE:
				setLocalVariable(varIndex, Double.parseDouble(args[i]));
				varIndex += 2;
				break;
			default:
				throw new IllegalArgumentException("Only primitive types for parameters of main(..) method supported.");
			}
		}
	}
	
	/**
	 * Add an int-like value (boolean, char, byte, short or int) to the
	 * local variable store at the provided index.
	 * Subclasses are responsible for implementing the local variable store
	 * and this operation.
	 * 
	 * @param index
	 * @param value
	 */
	abstract public void setLocalVariable(int index, int value);
	 ...
	 * @param value
	 */
	abstract public void setLocalVariable(int index, long value);
	 ...
	 * @param value
	 */
	abstract public void setLocalVariable(int index, float value);
	 ...
	 * @param value
	 */
	abstract public void setLocalVariable(int index, double value);
	
	/**
	 * Add a reference value to the
	 * local variable store at the provided index.
	 * Subclasses are responsible for implementing the local variable store
	 * and this operation.
	 * 
	 ...
	 * @param value
	 */
	abstract public void setLocalVariable(int index, Object value);
	
	/**
	 * Return the value that was provided to the return instruction of the
	 * interpreted method.
	 * If the method does not have a result value, this method returns null. 
	 * @return
	 */
	abstract public Object getResultValue();
