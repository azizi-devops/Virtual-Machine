...
//		File file = Path.of("target/test-classes/GGT2.class".replace('/', File.separatorChar)).toFile();
//		File file = Path.of("target/test-classes/Factorial.class".replace('/', File.separatorChar)).toFile();
//		File file = Path.of("target/test-classes/ManyBlocks.class".replace('/', File.separatorChar)).toFile();
		File file = Path.of("target/test-classes/Prime.class".replace('/', File.separatorChar)).toFile();

		Interpreter interpreter = new DecodeDispatchInterpreterProvided (file);
//		Interpreter interpreter = new IndirectThreadedIntepreter(file);
//		Interpreter interpreter = new DirectThreadedIntepreter(file);

		interpreter.setArguments(new String[] {"24", "24" });
		interpreter.run();
	}
